...
export function buildHomeQuoteReport(draft: QuoteDraft) {
  ...
  const sections = QUOTE_SECTIONS.map((section, index) => `<section id="${section.id}"><h2>${index + 3}. ${section.label}</h2><p class="source">Source: prospect / staff entry; verify against documents.</p><dl>${section.fields.map(field => row(field.label, draft.answers[field.key] || (draft.answers.policyStatus === 'No current policy' && ['carrier', 'coverageA', 'renewalDate', 'premium', 'deductibles'].includes(field.key) ? 'Not applicable - prospect reports no current policy' : ''))).join('')}</dl></section>`).join('');
