...
2. **Launch one of 45 agency tools** without hunting through bookmarks.
...
- **Client Folder / Cloud Folder → Google Drive (verified 2026-08-16).** Both live in `SearchCard.tsx` via `clientFolderSearchUrl()` / `clientFoldersRootUrl()`.
  ...
  - **Addressed by `authuser=Bill@billlayneinsurance.com`, never `/u/N/`.** Only that account is a member of the shared drive — `billlayneinsurance@gmail.com` and `docs@billlayneinsurance.com` both get "You need access". The numeric index is unstable: Bill has three Google accounts signed in and a `/u/3/` link was seen silently redirecting to `/u/0/`. `DRIVE_ACCOUNT_EMAIL` is the knob if the account ever changes.
  - Search uses **`title:`**, e.g. `…/drive/search?q=title:Dimmette&authuser=…`, so the client's own folder ranks first instead of being buried under every PDF that merely mentions the name. Percent-encoding is fine — Drive normalizes `title%3A` back to `title:`. **Word order does not matter**: typing "Brian Johnson" finds the "Johnson, Brian" folder, because Drive tokenizes titles.
  - **`parent:` does NOT work in the Drive web UI.** Tested against this Drive: zero results. The Location filter only offers Anywhere / My Drive / Shared with me / Starred / Trashed. There is no URL that pins a Drive web search to one folder — `title:` is the closest available. Do not "fix" this by reintroducing `parent:`.
  - **Cloud Folder** button: with a name typed it runs the same `title:` search; empty, it opens the BLI Clients root to browse everyone.
  - Superseded: an older `Client Folders` folder (`1lqq_4WpQ…`, "First Last" naming) in `billlayneinsurance@gmail.com`'s My Drive. It still exists but is NOT the live system.
...
**45 tools in 4 categories:** Operations (12) · Documents & Forms (18) · AI (11) · Property & Coverage (4). Sorted alphabetically within each section. **Agency Password Vault belongs in Operations.**
...
**Currently `LAUNCHER_VERSION = 7`** — v2: task board, mail gateway, Claude Quotes/Gmail · v3: `pdf-studio` · v4: `bli-auto-rater` · v5: `nc-grange-new-business` · v6: `sign-forms` · v7: `cancellation-request`. *(Verify in the file before bumping — see §0.)*
